...
class TimeStep {
  ...
  next() {
    ...
    switch (this.scale) {
      case 'millisecond':  this.current = add(this.current, { milliseconds: this.step }); break;
      case 'second':       this.current = add(this.current, { seconds: this.step }); break;
      case 'minute':       this.current = add(this.current, { minutes: this.step }); break;
      case 'hour':
        this.current = add(this.current, { hours: this.step });

        // DST should be handled elsewhere
    ...
    if (this.step != 1) {
      ...
      switch (this.scale) {
        ...
        case 'minute':       if(this.current.getMinutes() > 0 && this.current.getMinutes() < this.step) this.current = set(this.current, { minutes: 0 }); break;
  ...
  static snap(date, scale, step) {
    ...
    if (scale == 'year') {
      ...
      switch (step) {
        ...
        case 10:
          clone = set(clone, {
            minutes: Math.round(clone.getMinutes() / 5) * 5,
            seconds: 0
          });
          break;
        case 5:
  ...
  getLabelMajor(date) {
    ...
    const formatString = this.format.majorLabels[this.scale];
    return (formatString && formatString.length > 0) ? format(date, formatString) : '';
  }
  ...
  getClassName() {
    ...
    function today(date) {
      ...
      if (isSameDay(date, add(now, { days: 1 }))) {
        return 'vis-tomorrow';
      }
      if (isSameDay(date, sub(now, { days: 1 }))) {
    ...
    function currentWeek(date) {
      return isSameWeek(Date.now(), date) ? 'vis-current-week' : '';
    }
    ...
    function currentMonth(date) {
      return isSameMonth(Date.now(), date) ? 'vis-current-month' : '';
    }
    ...
    function currentYear(date) {
      return isSameYear(Date.now(), date) ? 'vis-current-year' : '';
    }
    ...
    switch (this.scale) {
      ...
      case 'minute':
        classNames.push(today(current));
        classNames.push(even(current.getMinutes()));
        break;
      case 'hour':
        ...
        break;
      case 'weekday':
        classNames.push(`vis-${format(current, 'EEEE').toLowerCase()}`);
        classNames.push(today(current));
        classNames.push(currentWeek(current));
        classNames.push(even(current.getDate()));
        break;
      case 'day':
        classNames.push(`vis-day${current.getDate()}`);
        classNames.push(`vis-${format(current, 'MMMM').toLowerCase()}`);
        classNames.push(currentMonth(current));
        if(this.step <= 2) {
          classNames.push(today(current));
          classNames.push(`vis-${format(current, 'EEEE').toLowerCase()}`);
        }
        classNames.push(even(current.getDate() - 1));
        break;
      case 'week':
        classNames.push(`vis-week${format(current, 'w')}`);
        classNames.push(currentWeek(current));
        classNames.push(even(getWeek(current)));
        break;
      case 'month':
        classNames.push(`vis-${format(current, 'MMMM').toLowerCase()}`);
        classNames.push(currentMonth(current));
        classNames.push(even(current.getMonth()));
        break;
      case 'year':
        classNames.push(`vis-year${current.getFullYear()}`);
        classNames.push(currentYear(current));
        classNames.push(even(current.getFullYear()));
        break;
    }
