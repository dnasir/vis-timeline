    ...
    if (Array.isArray(hiddenDates) == true) {
      for (let i = 0; i < hiddenDates.length; i++) {
        if (hiddenDates[i].repeat === undefined) {
          const start = new Date(hiddenDates[i].start);
          const end = new Date(hiddenDates[i].end);
          ...
          const dateItem = {};
          dateItem.start = start.valueOf();
          dateItem.end = end.valueOf();
...
export function updateHiddenDates(moment, body, hiddenDates) {
  ...
  if (hiddenDates && body.domProps.centerContainer.width !== undefined) {
    convertHiddenOptions(body, hiddenDates);
